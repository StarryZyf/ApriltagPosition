...
def acute_angle(side1,side2):
    side11 = abs(side1)
    side22 = abs(side2)
    angle = math.degrees(math.atan(side11/side22))
    if side1 > 0:
        angle = angle
    else:
        angle = -angle
...
def out_position(path=""):
    ...
    if longs > 0 :
        ...
        #angle
        angle_x = acute_angle(tvec[0],tvec[2])
        angle_z = acute_angle(tvec[1] ,pow((pow(tvec[0], 2) + pow(tvec[2],2)),0.5))
        # 打印结果
        print(tags[0]["id"])
